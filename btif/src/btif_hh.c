...
static void btif_hh_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_HH_LINK_EVT:
			if (p_data->conn.status == BTA_HH_OK) {
				HAL_CBACK(bt_hh_callbacks, connection_state_cb, (bt_bdaddr_t*) &p_data->conn.bda,BTHH_CONN_STATE_CONNECTING);			
			}else {			
				BTIF_TRACE_WARNING0("BTA_HH_LINK_EVT: Error");
			}
		    break;
        case BTA_HH_OPEN_EVT:
            ...
            if (p_data->conn.status == BTA_HH_OK) {
			    ...
			    else if (p_dev->host_reject == TRUE)
				{
					BTIF_TRACE_WARNING0("BTA_HH_OPEN_EVT: Error, host has send disconnect,skip that MSG,waiting for disconnection...");
