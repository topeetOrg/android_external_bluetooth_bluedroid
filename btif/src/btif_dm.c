                            ...
                            UINT8 *p_remote_name, UINT8 *p_remote_name_len)
{
    ...
    if (p_search_data->inq_res.p_eir)
    {
		...
		//Xiaomi long name(EIR SHORTEN LOCAL TYPE) will lead to chinese disorder
		//so we give up name of SHORTEN LOCAL TYPE,just get the COMPLETE LOCAL NAME by RNR
		/*
    ...
    switch (event)
    {
        ...
        case BTA_DM_INQ_RES_EVT:
        {
...

            {
                ...
                /* DEV_CLASS */
#if (defined(BLE_INCLUDED) && (BLE_INCLUDED == TRUE))
                //Android 4.4,only HOGP implemented
				if ((cod == COD_UNCLASSIFIED) && (p_search_data->inq_res.device_type == BT_DEVICE_TYPE_BLE))
				{
					cod = COD_HID_MAJOR;
				}				
#endif
...
static void btif_dm_search_services_evt(UINT16 event, char *p_param)
{
    ...
    switch (event)
    {
        case BTA_DM_DISC_RES_EVT:
        {
            ...
            //SDP fail:Remove device and Bonding fail
			if ((p_data->disc_res.result != BTA_SUCCESS) && (p_data->disc_res.num_uuids == 0) &&
				(pairing_cb.state == BT_BOND_STATE_BONDING ) && 
				(BTIF_DM_MAX_SDP_ATTEMPTS_AFTER_PAIRING <= pairing_cb.sdp_attempts ))
			{
				BTIF_TRACE_DEBUG1("%s Remote Service SDP done. SDP fail:Remove device and Bonding fail",
                                   __FUNCTION__);
