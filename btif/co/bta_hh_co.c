...
//fixed apple hid keyboard:Apple 1314 bug
static int first_input_data = 0;
static int apple_error_key_maybe = 0;
int bta_hh_co_write(int fd, UINT8* rpt, UINT16 len)
{
    APPL_TRACE_DEBUG0("bta_hh_co_data: UHID write");
    struct uhid_event ev;
	//fixed Apple 1314 bug which result in auto up focus
	//check received error input key:{0x13,0x03},{0x13,0x02}
    if (first_input_data) {  
	   first_input_data = 0;
       if ((len == 2) && (rpt[0] == 0x13) && 
	   	   (rpt[1] == 0x03) && (!apple_error_key_maybe)){
           apple_error_key_maybe = 1;
		   return 0;
	   }
	}else if (apple_error_key_maybe){
	   if ((len == 2) && (rpt[0] == 0x13) && (rpt[1] == 0x02))
	   {
          apple_error_key_maybe = 0;
		  return 0;
	   }else {
	       //write previous input data
           struct uhid_event ev0;
           ev0.type = UHID_INPUT;
           ev0.u.input.size = 2;
           if(2 > sizeof(ev0.u.input.data)){
               APPL_TRACE_WARNING1("%s:report size greater than allowed size",__FUNCTION__);
               return -1;
           }
		   ev0.u.input.data[0]=0x13;
		   ev0.u.input.data[1]=0x03;
           uhid_write(fd, &ev0);
	   }
	}
	apple_error_key_maybe = 0;
...
** Returns       void .
*******************************************************************************/
//Open UHID driver really
void bta_hh_co_open_hw(btif_hh_device_t * p_dev)
{   
    if (p_dev == NULL){
 		APPL_TRACE_ERROR0("NULL p_dev!!!");										
 		return;
    } 
...
*******************************************************************************/
btif_hh_device_t * bta_hh_co_open(UINT8 dev_handle, UINT8 sub_class, tBTA_HH_ATTR_MASK attr_mask,
                    UINT8 app_id)
{
    UINT32 i;
    ...
    if (p_dev == NULL) {
        ...
        for (i = 0; i < BTIF_HH_MAX_HID; i++) {
            if (btif_hh_cb.devices[i].dev_status == BTHH_CONN_STATE_UNKNOWN) {
                ...
                //Added by Yuebing 2013-11-22 For cannot open uhid driver when connect to a hid device first time.
                p_dev->fd = -1;
