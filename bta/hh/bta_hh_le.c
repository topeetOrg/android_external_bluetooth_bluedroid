...
void bta_hh_le_input_rpt_notify(tBTA_GATTC_NOTIFY *p_data)
{
    ...
    //No report ID,do not append report ID to the head of data
	if (p_rpt->rpt_id == 0)
	{
		/* need to append report ID to the head of data */
		if ((p_buf = (UINT8 *)GKI_getbuf((UINT16)(p_data->len))) == NULL)
		{
		...
		memcpy(&p_buf[0], p_data->value, p_data->len);
	}
	else 
	{
		/* need to append report ID to the head of data */
...
static void bta_hh_gattc_callback(tBTA_GATTC_EVT event, tBTA_GATTC *p_data)
{
    ...
    switch (event)
    {
        ...
        case BTA_GATTC_DEREG_EVT: /* 1 */
		    /* terrence,2013/12/28,make sure no live connection(legacy),othersise hid bta statemachine maybe broken */
