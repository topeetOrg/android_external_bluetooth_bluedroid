...
void bta_hh_open_cmpl_act(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
    ...
    p_dev = bta_hh_co_open(p_cb->hid_handle, p_cb->sub_class,
                       p_cb->attr_mask,  p_cb->app_id);
		
	if (p_dev != NULL)
	{  
	   bdcpy(p_dev->bd_addr.address,p_cb->addr);
	   p_dev->dev_status = BTHH_CONN_STATE_CONNECTED;
       //Notify HID service that link is up before HID driver opened
	   conn.status = BTA_HH_OK;
       (* bta_hh_cb.p_cback)(BTA_HH_LINK_EVT, (tBTA_HH *)&conn);
	   usleep(500000); /* 500milliseconds */
	
	   //Open UHID driver
	   bta_hh_co_open_hw(p_dev);
	}
