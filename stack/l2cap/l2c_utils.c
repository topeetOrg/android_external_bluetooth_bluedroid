...
BT_HDR *l2cu_build_header (tL2C_LCB *p_lcb, UINT16 len, UINT8 cmd, UINT8 id)
{
	...
	//BLE data:host to controller,must be Non-flush 
	#if (BLE_INCLUDED == TRUE)
	if (p_lcb->is_ble_link)
	{
...
BOOLEAN l2cu_set_acl_priority (BD_ADDR bd_addr, UINT8 priority, BOOLEAN reset_after_rs)
{
	...
	L2CAP_TRACE_API6 ("SET ACL PRIORITY  bdaddr: %02x%02x%02x%02x%04x, reset_after_rs:%d",
                    bd_addr[0], bd_addr[1], bd_addr[2],
                    bd_addr[3], (bd_addr[4] << 8) + bd_addr[5], reset_after_rs);
	...
	//Revision for Mediatek A2DP performance
    //if (BTM_IS_BRCM_CONTROLLER())
    {
        ...
        {
            //No need send HCI command for mediatek chip
            //pp = command;
	...
	APPL_TRACE_EVENT1("set ACL priority as %d ", p_lcb->acl_priority);
    return(TRUE);
}
