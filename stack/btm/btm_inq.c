...
static void btm_initiate_inquiry (tBTM_INQUIRY_VAR_ST *p_inq)
{
	...
	//Adapter controller behavior
	UINT8 scatter_ratio = 1;
	if (BTM_GetNumAclLinks() <= 3){

		scatter_ratio = BTM_GetNumAclLinks() + 1;
	}else{
	
		scatter_ratio = 4;
	}

	p_inqparms->duration *= scatter_ratio;

	BTM_TRACE_API2 ("Link num: %d,New Inquiry duaration: %d",BTM_GetNumAclLinks(),p_inqparms->duration);
