...
void BTM_ConfirmReqReply(tBTM_STATUS res, BD_ADDR bd_addr)
{
	...
	//Passkey Entry pairing cancelled by diallog negative button
	//1.Disconnect the acl link
	//2.Then p_auth_complete_callback will notify to showUnbondMessage()
...
void btm_sec_conn_req (UINT8 *bda, UINT8 *dc)
{
    ...
    {   
        //check if we are not in collision situation,otherwise,accept the connection request
        p_lcb = l2cu_find_lcb_by_bd_addr (bda);
		if (!(p_lcb && (p_lcb->disc_reason == HCI_ERR_CONNECTION_EXISTS) && 
...
void btm_sec_auth_complete (UINT16 handle, UINT8 status)
{
	...
	UINT8              old_flags = btm_cb.pairing_flags;
    ...
    */
	...
	//Check Whether confirm cancelld
    if ((BTM_PAIR_FLAGS_WE_CANCEL_DD & old_flags) && 
		(p_dev_rec->sec_state == BTM_SEC_STATE_AUTHENTICATING) &&
    ...
    else if (btm_cb.api.p_auth_complete_callback)
    {
