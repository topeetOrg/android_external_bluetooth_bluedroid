...
static int btsnoop_log_open(char *btsnoop_logfile)
{
#if defined(BTSNOOPDISP_INCLUDED) && (BTSNOOPDISP_INCLUDED == TRUE)
    hci_btsnoop_fd = -1;
	static int btsnoop_file_dupped = 0;

    struct stat st;//wenhui
    int is_log_exist;

    SNOOPDBG("btsnoop_log_open: snoop log file = %s\n", btsnoop_logfile);

    if(stat(btsnoop_logfile, &st)!=0){
	ALOGE("btsnoop log file does not exist");
	    is_log_exist =0;
    } else {
	ALOGE("btsnoop log file does exist");
	    is_log_exist =1;
    }	
    ...
    if ((btsnoop_logfile != NULL) && (strlen(btsnoop_logfile) != 0))
    {
        ...
        if (!btsnoop_file_dupped) {
            dup(hci_btsnoop_fd);
            btsnoop_file_dupped = 1;
		    ALOGD("btsnoop file duplicated for debug");
        }
		#endif
    	if(is_log_exist==0)
